...
function cargarDatos() {
    ...
    AjaxGet("frmCategoria.aspx/mtdListarCategoria",
        function (response) {
...
$('#tbCategoria tbody').on('click', 'button[class="btn btn-sm btn-danger"]', function () {
    ...
    swal({
        ...
        AjaxPost("frmCategoria.aspx/mtdEliminarcategoria", JSON.stringify(request),
            function (response) {
                if (response.estado) {
                    cargarDatos();
                    swal.close();
...
$('#btnGuardarCambios').on('click', function () {
    ...
    if (!camposvacios) {
        ...
        if (parseInt($("#txtIdCategoria").val()) == 0) {
            ...
            AjaxPost("frmCategoria.aspx/mtdRegistrarCategoria", JSON.stringify(request),
                function (response) {
            ...
            AjaxPost("frmCategoria.aspx/mtdEditarCategoria", JSON.stringify(request),
                function (response) {
                    $(".modal-body").LoadingOverlay("hide");
                    if (response.estado) {
                        cargarDatos();
